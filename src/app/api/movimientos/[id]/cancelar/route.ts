...
const cancelarSchema = z.object({
  motivo: z.string().min(1, 'Motivo requerido').optional(),
});
...
export async function PATCH(request: Request, context: { params: { id: string } }) {
  ...
  if (auth.user!.rol !== 'Dueño' && auth.user!.rol !== 'Admin') {
    return forbiddenResponse('Solo Dueño y Admin pueden cancelar movimientos');
  }
  ...
  try {
    ...
    const validated = cancelarSchema.parse(body);
    ...
    const cancelado = await movimientoService.cancelarMovimiento(movimientoId, auth.user!.userId, validated.motivo);
    return NextResponse.json({ success: true, data: cancelado, tenant });
  } catch (error) {
