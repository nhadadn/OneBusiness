...
const pagarSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    const parsed = new Date(data.fechaPago);
...
export async function PATCH(request: Request, context: { params: { id: string } }) {
  ...
  if (auth.user!.rol !== 'Dueño' && auth.user!.rol !== 'Admin') {
    return forbiddenResponse('Solo Dueño y Admin pueden marcar pagos');
  }
  ...
  try {
    ...
    const validated = pagarSchema.parse(body);
    ...
    const pagado = await movimientoService.marcarPagado(movimientoId, auth.user!.userId, validated.fechaPago);
    return NextResponse.json({ success: true, data: pagado, tenant });
  } catch (error) {
