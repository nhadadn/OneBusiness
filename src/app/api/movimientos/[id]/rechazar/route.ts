...
export async function PATCH(request: Request, context: { params: { id: string } }) {
  ...
  if (auth.user!.rol !== 'Dueño' && auth.user!.rol !== 'Admin') {
    return forbiddenResponse('Solo Dueño y Admin pueden rechazar movimientos');
  }
  ...
  try {
    ...
    const validated = rechazarSchema.parse(body);
    ...
    const rechazado = await movimientoService.rechazar({
      movimientoId,
      aprobadoPor: auth.user!.userId,
      motivoRechazo: validated.motivoRechazo,
    });
    return NextResponse.json({ success: true, data: rechazado, tenant });
  } catch (error) {
