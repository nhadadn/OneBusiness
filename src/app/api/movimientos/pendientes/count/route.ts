...
export async function GET(request: Request) {
  ...
  if (auth.user!.rol !== 'Dueño' && auth.user!.rol !== 'Admin') {
    return forbiddenResponse('Solo Dueño y Admin pueden ver pendientes');
  }

  try {
    const tenant = validateTenantAccess(request, auth.user!);
    const negocioIds = tenant.negociosAccesibles;
    const pendientes = await movimientoService.contarPendientes(negocioIds);
    const count = pendientes.reduce((acc, row) => acc + (Number(row.count) || 0), 0);

    return NextResponse.json({ success: true, count, tenant });
  } catch (error) {
