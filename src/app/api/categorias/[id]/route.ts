...
function handleServiceError(error: unknown): NextResponse {
  ...
  if (message.includes('inválido') || message.includes('desactivada')) {
...
export async function GET(request: Request, context: { params: { id: string } }) {
  ...
  try {
    ...
    const categoria = await categoriaService.obtenerCategoriaPorId(categoriaId, tenant.negocioId);
...
export async function PUT(request: Request, context: { params: { id: string } }) {
  ...
  try {
    ...
    const validated = actualizarCategoriaSchema.parse(body);

    const categoria = await categoriaService.actualizarCategoria(categoriaId, tenant.negocioId, auth.user!.rol, validated);
    return NextResponse.json({ success: true, data: categoria });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json({ success: false, error: 'Datos inválidos', details: error.issues }, { status: 400 });
...
export async function DELETE(request: Request, context: { params: { id: string } }) {
  ...
  try {
    ...
    const categoria = await categoriaService.desactivarCategoria(categoriaId, tenant.negocioId, auth.user!.rol);
    return NextResponse.json({ success: true, data: categoria });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json({ success: false, error: 'Datos inválidos', details: error.issues }, { status: 400 });
