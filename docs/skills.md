...
tipoMovimientoEnum:   'INGRESO' | 'EGRESO' | 'TRASPASO\_SALIDA' | 'TRASPASO\_ENTRADA'
estadoMovimientoEnum: 'PENDIENTE' | 'APROBADO' | 'RECHAZADO'
tipoCuentaEnum:       'EFECTIVO' | 'BANCARIA' | 'CAJA\_CHICA'
...
(table) => ({
negocioIdIdx: index('idx\_tabla\_negocio\_id').on(table.negocioId),
activoIdx:    index('idx\_tabla\_activo').on(table.activo),
estadoIdx:    index('idx\_tabla\_estado').on(table.estado),
...
import type { CuentaBanco } from '@/types/cuenta\_banco.types';
...
// Body POST/PATCH:
const createSchema = z.object({
tipo:         z.enum(\['INGRESO', 'EGRESO', 'TRASPASO\_SALIDA', 'TRASPASO\_ENTRADA']),
...
- SUM(parseFloat(m.monto) WHERE m.tipo = 'TRASPASO\_ENTRADA' AND m.estado = 'APROBADO')

* SUM(parseFloat(m.monto) WHERE m.tipo = 'EGRESO'           AND m.estado = 'APROBADO')
* SUM(parseFloat(m.monto) WHERE m.tipo = 'TRASPASO\_SALIDA'  AND m.estado = 'APROBADO')
...
.filter(m => m.tipo === 'INGRESO' || m.tipo === 'TRASPASO\_ENTRADA')
.reduce((sum, m) => sum + parseFloat(m.monto), 0);

const salidas = aprobados
.filter(m => m.tipo === 'EGRESO' || m.tipo === 'TRASPASO\_SALIDA')
.reduce((sum, m) => sum + parseFloat(m.monto), 0);
...
): 'CUADRADO' | 'SOBRANTE' | 'FALTANTE' | 'SIN\_SALDO\_REAL' {
if (saldoReal === null) return 'SIN\_SALDO\_REAL';
...
1. TRASPASO\_SALIDA  → negocio/cuenta ORIGEN
2. TRASPASO\_ENTRADA → negocio/cuenta DESTINO
...
const ESTADO\_STYLES: Record\<string, string> = {
...
\<Badge className={ESTADO\_STYLES\[estado] ?? ''}>{estado}</Badge>
